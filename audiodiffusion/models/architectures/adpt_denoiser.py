...
from .transformer_adpt import TransformerDecoderLayer_w_Adapter, TransformerDecoder_w_Adapter

class Adpt_Denoiser(nn.Module):
    ...
    def __init__(self,
        ...
        self.memory_bi_bias = init_bi_biased_mask(max_len)
        if tgt_attn_scale < 0.0: # means we only use the causal attention
            self.target_bi_bias = init_bi_biased_mask(max_len)
        ...
        # style embedding
        self.obj_vector = nn.Linear(id_dim, latent_dim * num_layers, )
